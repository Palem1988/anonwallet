...
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-binaries-anonwallet-v$version.tar.gz
Remove-Item -Force -ErrorAction Ignore ./artifacts/linux-deb-anonwallet-v$version.deb
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-binaries-anonwallet-v$version.zip
Remove-Item -Force -ErrorAction Ignore ./artifacts/Windows-installer-anonwallet-v$version.msi
Remove-Item -Force -ErrorAction Ignore ./artifacts/macOS-anonwallet-v$version.dmg
...
bash src/scripts/mkmacdmg.sh --qt_path ~/Qt/5.11.1/clang_64/ --version $version --anon_path ~/github/anon 
if (! $?) {
    Write-Output "[Error]"
    exit 1;
}
...
scp -r src/ singleapplication/ res/ ./anon-qt-wallet.pro ./application.qrc ./LICENSE ./README.md ${server}:/tmp/zqwbuild/ | Out-Null
...
if (! (Test-Path ./artifacts/linux-binaries-anonwallet-v$version.tar.gz) -or
    ! (Test-Path ./artifacts/linux-deb-anonwallet-v$version.deb) -or
    ! (Test-Path ./artifacts/Windows-binaries-anonwallet-v$version.zip) -or
    ! (Test-Path ./artifacts/macOS-anonwallet-v$version.dmg) -or 
    ! (Test-Path ./artifacts/Windows-installer-anonwallet-v$version.msi) ) {
        Write-Host "[Error]"
        exit 1;
    }
Write-Host "[OK]"
