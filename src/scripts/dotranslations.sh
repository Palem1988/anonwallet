...
rm -f res/*.qm
$QT_STATIC/bin/lrelease anon-qt-wallet.pro
...
ls res/*.qm | awk -F '[_.]' '{print $4}' | while read -r language ; do 
    if [ -f $QT_STATIC/translations/qtbase_$language.qm ]; then
        $QT_STATIC/bin/lconvert -o res/anon_$language.qm $QT_STATIC/translations/qtbase_$language.qm res/anon_qt_wallet_$language.qm
        mv res/anon_$language.qm res/anon_qt_wallet_$language.qm
    fi
done
